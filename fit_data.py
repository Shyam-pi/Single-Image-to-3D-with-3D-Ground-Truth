...
# python fit_data.py --type 'point' --lr 4e-3 --max_iter 20000
...
def fit_mesh(mesh_src, mesh_tgt, args, device):
    ...
    for step in range(start_iter, args.max_iter):
        ...
        if step % 100 == 0: print("[%4d/%4d]; ttime: %.0f (%.2f); loss: %.3f" % (step, args.max_iter, total_time,  iter_time, loss_vis))        
    
    mesh_src.offset_verts_(deform_vertices_src)

    print('Done!\nStarting Rendering')

    mesh_src = preprocess_mesh(mesh_src)
    mesh_tgt = preprocess_mesh(mesh_tgt)

    render_360(item = mesh_src, type3d = 'mesh', save_path = "outputs/fit_data/fit_data_mesh_pred.gif")
    render_360(item = mesh_tgt, type3d = 'mesh', save_path = "outputs/fit_data/fit_data_mesh_gt.gif")
...
def fit_pointcloud(pointclouds_src, pointclouds_tgt, args):
    ...
    pointclouds_src = preprocess_pcl(pointclouds_src)
    pointclouds_tgt = preprocess_pcl(pointclouds_tgt)

    render_360(item = pointclouds_src, type3d = 'point', save_path = "outputs/fit_data/fit_data_point_pred.gif")
    render_360(item = pointclouds_tgt, type3d = 'point', save_path = "outputs/fit_data/fit_data_point_gt.gif")
...
def fit_voxel(voxels_src, voxels_tgt, args):
    ...
    mesh_src = get_mesh_from_voxels(voxels_src)
    mesh_tgt = get_mesh_from_voxels(voxels_tgt)

    render_360(item = mesh_src, type3d = 'mesh', save_path = "outputs/fit_data/fit_data_voxel_pred.gif")
    render_360(item = mesh_tgt, type3d = 'mesh', save_path = "outputs/fit_data/fit_data_voxel_gt.gif")
